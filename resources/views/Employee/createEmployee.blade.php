    ...
    <div class="p2 mx-auto" style="width:50%">
        ...
        <form class="form-floating" method="POST" action="/employees">
            ...
            <div class="form-group">
                ...
                <input type="text" name="phone_number" id="phone_number" class="form-control mb-2" value="{{old('phone_number')}}">
                @error('phone_number')
                    <div class="error mb-3 bg-danger text-light p-2 rounded">{{ $message }}</div>
                @enderror
                <label for="address">@lang('employee.column.address') <span style="color: red">*</span></label>
                <input type="text" name="address" id="address" class="form-control mb-2" value="{{old('address')}}">
                @error('address')
                    <div class="error mb-3 bg-danger text-light p-2 rounded">{{ $message }}</div>
                @enderror
                ...
                <select name="gender" id="gender" class="form-select form-select-sm p-2 mb-2">
                    ...
                    <option value="Laki-Laki" @if(old('gender') == "Laki-Laki") selected @endif>@lang('employee.gender.male')</option>
                    <option value="Perempuan" @if(old('gender') == "Perempuan") selected @endif>@lang('employee.gender.female')</option>
                </select>
                @error('gender')
                    <div class="error mb-3 bg-danger text-light p-2 rounded">{{ $message }}</div>
                @enderror
